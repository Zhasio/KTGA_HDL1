...
WHEN ( SELECT DATE_FORMAT(КН.ДАТА - INTERVAL 1 DAY ,'%Y-%m-%d') FROM prykazy КН WHERE КН.PERSON_ID=П.PERSON_ID AND КН.`ГруппаПриказа` IN(1,2,3) AND КН.`Дата` > П.`Дата`  ORDER BY КН.`Дата` LIMIT 1) IS NOT NULL 
THEN ( SELECT  DATE_FORMAT(КН.ДАТА - INTERVAL 1 DAY ,'%Y-%m-%d') FROM prykazy КН 
WHERE КН.PERSON_ID=П.PERSON_ID 
AND КН.`ГруппаПриказа` IN(1,2,3) AND КН.`Дата` > П.`Дата`   ORDER BY КН.`Дата` LIMIT 1) ELSE '4712-12-31' END
